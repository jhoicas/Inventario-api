...
func Router(app *fiber.App, deps RouterDeps) {
	...
	if deps.RBACUC != nil {
		moduleHandler := NewModuleHandler(deps.RBACUC)
		superAdminGroup.Get("/modules", moduleHandler.ListModules)
	...
	if deps.CRMHandler != nil {
		...
		crmGroup.Post("/remarketing/send-batch", h.SendRemarketingBatch)
		crmGroup.Get("/import/status/:jobID", RequireRole(entity.RoleAdmin, "gerente"), h.GetImportStatus)
		crmGroup.Get("/customers", h.ListCustomers)
		crmGroup.Post("/customers", h.CreateCustomer)
		crmGroup.Get("/customers/:id", h.GetCustomerByID)
		...
		crmGroup.Post("/import/preview", RequireRole(entity.RoleAdmin, "gerente"), h.PreviewImport)
		...
		crmGroup.Get("/campaigns", h.ListCampaigns)
		crmGroup.Get("/campaigns/:id", h.GetCampaignByID)
		crmGroup.Post("/campaigns", h.CreateCampaign)
		crmGroup.Put("/campaigns/:id", h.UpdateCampaign)
		crmGroup.Delete("/campaigns/:id", h.DeleteCampaign)
		crmGroup.Post("/campaigns/:id/execute", h.ExecuteCampaign)
		crmGroup.Post("/campaigns/send", h.SendCampaign)
		crmGroup.Post("/campaigns/send-test", h.SendTestCampaign)
		crmGroup.Post("/campaigns/test-message", h.SendTestMessage)
		crmGroup.Get("/campaigns/:id/metrics", h.GetCampaignMetrics)
		crmGroup.Get("/audit-logs", h.ListAuditLogs)
		...
		crmGroup.Put("/campaign-templates/:id", h.UpdateCampaignTemplate)
